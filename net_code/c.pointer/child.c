...
** child.c
**
**	Run a function as a child process; the caller is responsible
**	for waiting.
*/

static char sccs_id_distr[] = "@(#)child.c	1.3      10/15/97";

#include "try.h"
#include "data.h"

void
child_proc( void (*id_to_become)( void ), void (*func_to_call)(), int a0, int a1 )
{
	fflush( stdout );
	fflush( stderr );
	switch( fork() ){
	case -1:
		error( ERR_IMPOS + 1, "Can't fork!\n" );
		/*NOTREACHED*/

	case 0:
		logging = FALSE;
		if( id_to_become != NULL )
			(*id_to_become)();
		(*func_to_call)( a0, a1 );
		error( ERR_IMPOS + 1, "Child returned instead of exiting!\n" );
		/*NOTREACHED*/
